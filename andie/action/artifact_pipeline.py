...
import asyncio, json, uuid, re, logging
...
WORKSPACE_ROOT = Path("/app/workspace/artifacts")
...
def _preprocess_raw(raw: str) -> str:
    ...
    # Pass 1: Replace short inline template expressions `${expr}` first
    # so they don't break the outer backtick block matching
    raw = re.sub(r"`(\$\{[^}]*\})`", bt_to_dq, raw)
    # Pass 2: Replace outer multi-line backtick strings
...
async def run_artifact_pipeline(
    target: str, ollama_chat_fn, system_prompt: str
) -> dict[str, Any]:
